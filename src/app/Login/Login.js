...
import loginImage from '../../assets/images/login-img-6-02.png';
function Login() {
...
<div className="row" style={{paddingTop:"20px"}}></div>
  <div className="container" style={{margin: "0 auto", boxShadow: "rgb(248 244 252) 0px 7px 29px 0px", borderRadius: "5px"}}>
        ...
        <div className="row g-2" style={{padding:"20px 0px 20px 0px", margin:"0 auto"}}>
          <div className="col-6" id="login">
            <div className="p-3" >
             <img src={loginImage} width="540px"/>

            </div>
          </div>
          <div className="col-6" id="login-box">
            <div className="p-3 ">
                <div className="row" >
                    <svg id="logo" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 112.837 137.566">
                    ...
                    <div className="row" style={{margin:"0 auto", marginBottom: "5px"}} >
                       <span style={{textAlign: "center",margin:"0 auto"}}> 
                         <h3 style={{fontWeight: "normal", color: "#380d53"}}>License Portal</h3></span> 
                   </div>
                  
{ comments ? <p className="row px-3 inputContainer" style={{color:"red",fontSize:".7em",width: "76%", margin: "0 auto", padding: "10px"}}>Invalid Email Address or Password</p>: null }
                         <form onSubmit={handleSubmit} noValidate>
                   <div className="row px-3 inputContainer"  style={{width: "76%", margin: "0 auto", padding: "10px"}}> <i className="fa fa-user icon"></i>
                    <input autoComplete="off" className={`input ${errors.email && 'is-danger'}`} type="email" name="email" onChange={handleChange} value={values.email || ''} required placeholder="Enter a valid email address"></input>{errors.email && (
                    <p style={{color:"red",fontSize:".7em"}}>{errors.email}</p>
                  )}</div>
                    <div className="row px-3 inputContainer" style={{width: "76%", margin: "0 auto", padding: "10px"}}><i className="fa fa-key icon"></i>
                 <input autoComplete="off" className={`input ${errors.password && 'is-danger'}`} type="password" name="password" onChange={handleChange} value={values.password || ''} required placeholder="Enter password"></input>{errors.password && (
                  <p style={{color:"red",fontSize:".7em"}}>{errors.password}</p>
                )}</div>
                    <div className="row px-3 mb-4" style={{width: "76%", margin: "0 auto", padding: "10px"}}>
                <div className="custom-control custom-checkbox custom-control-inline"> 
            <input id="chk1" type="checkbox" name="chk" className="custom-control-input"/>
            <label htmlFor="chk1" className="custom-control-label text-sm"><span>Remember me</span> </label> </div>
        <a href="#" className="ml-auto mb-0 text-sm"><span>Forgot Password?</span></a>
        </div>
        <div className="row mb-3 px-3" style={{width: "76%", margin: "0 auto", padding: "10px"}}> <button type="submit" className="gradient-buttons btn text-center">Login</button> </div>
        </form></div>
       </div>
